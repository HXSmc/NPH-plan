...
- **AI-4 eval harness has never scored a real model pass** (§2.9, §1.11) — blocked on the missing synthetic HTML→PDF rasterizer. Treat the 98%/95% accuracy thresholds in `packages/ai/evals/scoring.ts` as targets, not proven results.
- **AI-4's arithmetic validator has no adjustment/withholding bucket.** The extraction schema models four amounts per claim line (billed, paid, patient-share, rejected) and the validator requires them to cross-total. A real-world remittance that includes a **contractual adjustment/write-off** (a fifth bucket insurers commonly use) won't cross-total against that four-field model — it would fail validation and could become permanently stuck at "reject only," never approvable, even though the extraction might be perfectly accurate. This is a real-data-enablement gap to close before BLK-AI-1 clears, not a bug in today's synthetic-only build (the synthetic corpus doesn't generate adjustment scenarios).
