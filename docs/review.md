...
**AI-4 eval harness — can now run a real scored pass (both former blockers cleared), but the accuracy numbers still need a human to read them.** `packages/ai/evals/extractEob.eval.ts` is gated behind `AI_EVALS_LIVE=1` (never runs in CI, never runs in `pnpm test`) and is meant to score real extraction accuracy against the synthetic corpus's ground truth (`EVAL_TARGET_THRESHOLDS`: amounts ≥98%, overall ≥95%). The two things that used to stop it from running are both resolved: (1) the HTML→PDF rasterizer it needs is **built** (`test/synthetic-eob/src/rasterize.ts`, headless Chromium — landed 2026-07-10; the old `TODO(ai-route)` seam in `generate.ts` is gone), and (2) an `ANTHROPIC_API_KEY` is now **configured**, so the live Anthropic calls the eval makes can actually happen. So a `AI_EVALS_LIVE=1` run (key + `DATABASE_URL` present) now executes end to end against the synthetic corpus and writes per-tier scores to `packages/ai/evals/.output/`. **Two caveats remain, both by design:** the harness's own `expect(...)` assertions deliberately check only *plumbing* (one report row per tier, a score recorded per document) and **never** assert the 98%/95% thresholds — those are printed for a human to compare against, not gated on, so routine model drift doesn't masquerade as a broken build. And **no scored run has actually been executed and reviewed yet** — until someone runs it and reads `.output/extractEob-*.json`, treat 98%/95% as targets, not proven results. A green run of this file still does **not** by itself mean "the model is accurate."
...
- **Live eval harness (AI-4 only, `packages/ai/evals/*.eval.ts`):** gated behind `AI_EVALS_LIVE=1`, never runs in CI or `pnpm test`. **UPDATE 2026-07-10 (AI-4 real-data-gaps unit):** the HTML→PDF rasterizer this section used to say was missing is now built (`test/synthetic-eob/src/rasterize.ts`, Playwright headless Chromium, `test/synthetic-eob/test/rasterize.test.ts` proves a genuine `%PDF-`-magic-byte PDF renders for both an EN and an `arabicHeavy` RTL scenario) and `extractEob.eval.ts`'s `PdfRenderer` seam is wired to it — the harness can now actually run `AI_EVALS_LIVE=1` end to end. **UPDATE (API key added):** an `ANTHROPIC_API_KEY` is now configured, so the last external prerequisite for a live run (`AI_EVALS_LIVE=1` + a key + `DATABASE_URL`) is satisfied — nothing now blocks a real scored pass; it just hasn't been *executed and reviewed* yet (a deliberate manual/CI-scheduled step, not part of this build session). Until it is, the 98%/95% target thresholds in `packages/ai/evals/scoring.ts` remain targets, not proven results — but both former blockers (the missing rasterizer, then the absent key) are now gone.
...
- **AI-4 eval harness has never scored a real model pass** (§2.9, §1.11) — but nothing external blocks it anymore: the rasterizer that blocked it is built (see §2.9's 2026-07-10 update) *and* an `ANTHROPIC_API_KEY` is now configured. A real `AI_EVALS_LIVE=1` scored run is still outstanding purely because no one has executed and reviewed it yet (a deliberate manual/CI-scheduled step, not a code gap). Treat the 98%/95% accuracy thresholds in `packages/ai/evals/scoring.ts` as targets, not proven results, until that run happens.
...
| `AI_EVALS_LIVE=1` scored eval run | `packages/ai/evals/scoring.ts`, `packages/ai/evals/extractEob.eval.ts` | ~~**BLK-AI-2** (needs a live key)~~ **key now added** — synthetic-corpus eval is unblocked; only manual execution + review remains | The 98%/95% accuracy thresholds become proven results instead of unscored targets |
