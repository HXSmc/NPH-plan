...
- **To test it, you need a PDF file to upload.** Unlike Ingest's JSON sample bundle, **there is currently no generator that produces a ready-made test PDF** — the synthetic-EOB corpus (`test/synthetic-eob`) only generates the *ground-truth data and an HTML template* for each scenario; rendering that HTML to an actual PDF is a documented, not-yet-built step (`TODO(ai-route)` in `generate.ts`). So to exercise this tab end-to-end, bring your own PDF (any real or dummy PDF works for a plumbing test — the upload only checks for valid `%PDF-` magic bytes before proceeding; for a *meaningful* extraction test you'd want a PDF that actually looks like a remittance).
- **Steps:**
...
**AI-4 eval harness — exists, but has never actually scored a model.** `packages/ai/evals/extractEob.eval.ts` is gated behind `AI_EVALS_LIVE=1` (never runs in CI, never runs in `pnpm test`) and is meant to score real extraction accuracy against the synthetic corpus's ground truth (`EVAL_TARGET_THRESHOLDS`: amounts ≥98%, overall ≥95%). **It cannot run a real pass today**: the synthetic-EOB generator (`test/synthetic-eob`) only produces ground-truth data plus an HTML template per scenario, and rendering that HTML into an actual PDF is a documented, not-yet-built step (`TODO(ai-route)`). Until that rasterizer exists, this harness's assertions only prove the scoring *plumbing* works, not that the model hits its accuracy targets — don't read a green run of this file as "the model is accurate," and don't treat the 98%/95% numbers as anything but aspirational targets yet.
...
- **Live eval harness (AI-4 only, `packages/ai/evals/*.eval.ts`):** gated behind `AI_EVALS_LIVE=1`, never runs in CI or `pnpm test`. **UPDATE 2026-07-10 (AI-4 real-data-gaps unit):** the HTML→PDF rasterizer this section used to say was missing is now built (`test/synthetic-eob/src/rasterize.ts`, Playwright headless Chromium, `test/synthetic-eob/test/rasterize.test.ts` proves a genuine `%PDF-`-magic-byte PDF renders for both an EN and an `arabicHeavy` RTL scenario) and `extractEob.eval.ts`'s `PdfRenderer` seam is wired to it — the harness can now actually run `AI_EVALS_LIVE=1` end to end. It still has not been run against the real Anthropic API (that requires `AI_EVALS_LIVE=1` + a key + `DATABASE_URL`, a deliberate manual/CI-scheduled step, not part of this build session), so the 98%/95% target thresholds in `packages/ai/evals/scoring.ts` remain targets, not proven results — but the blocker was the missing rasterizer, and that blocker is now gone.
...
- **AI-4 eval harness has never scored a real model pass** (§2.9, §1.11) — the rasterizer that blocked this is now built (see §2.9's 2026-07-10 update); a real `AI_EVALS_LIVE=1` scored run against the Anthropic API is still outstanding (a deliberate manual/CI-scheduled step, not a code gap). Treat the 98%/95% accuracy thresholds in `packages/ai/evals/scoring.ts` as targets, not proven results, until that run happens.
