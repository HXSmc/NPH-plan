...
- **What to do (remaining):**
  ...
  2. ~~Email CHI DMO~~ **SENT 2026-07-20 to `info@chi.gov.sa`; auto-acknowledged same day** — generic reply: *"تم إحالة طلبكم الى القسم المختص"* (your request has been forwarded to the relevant department) — no data yet, no timeline given. Follow up by phone (19977 / 920001177) if no substantive reply in ~1-2 weeks.
...
- **✅ HealthOrbit replied 2026-07-20 (Jobin Jose, Co-Founder) — real, material downgrade of the competitive-threat read:** their Claim Scrubber and Medical Coding Engine (the products relevant to denial-management) are **built for the UAE market only** — *"we have not yet released the Saudi NPHIES version... currently in development and planned for release in Q4 2026."* So the "validation-tier NPHIES depth" found in the 2026-07-01 research pass wasn't even live in the Saudi market when it was checked — it's a UAE product, not a KSA one. **Separately, their AI Clinical Documentation Scribe (a different product — ambient clinical notes, not claims/denial-management) is already live in Saudi Arabia, with Aster Hospitals as a customer** — worth knowing as a data point on who's active in KSA healthtech, but not a competitor to Taweed's actual product. No pricing given — instead they asked 3 discovery questions before offering a demo (already working with Saudi providers? existing denial-mgmt customers or still building? technology partner or product-integration interest?). **A reply draft answering those honestly is in Gmail** — the honest answer is Taweed is pre-revenue/still building and isn't looking to integrate their (not-yet-live) Saudi product, but worth reconnecting closer to their Q4 2026 launch to see if they become a real competitor.
- **What to do (remaining):** review + send the HealthOrbit reply draft; send Email 11 (Ecaresoft) — no response yet; re-confirm Ecaresoft's Claims module is now fully live (2021 doc may be stale) — folded into Email 11's question 1.
...
- **🆕 CR economic activity — 2026-07-20, verified live on `mc.gov.sa`'s official ISIC4 guide (searched the actual portal, not recited from memory):** recommend **`620111` — تطوير التطبيقات (Application Development)**, under section المعلومات والاتصالات (Information & Communication) → group 6201 أنشطة البرمجة الحاسوبية (Computer Programming Activities). This is the clean, standard software-product activity — one main activity, not a bundle (`620113` AI Technologies is a defensible add but not necessary; the AI features are part of the one product, not a separate business line).
...
- **What to do:** ~~Register as a taxpayer at CR issuance regardless.~~ **DONE 2026-07-22** — general ZATCA taxpayer registration completed. VAT/Fatoora still not due (pre-revenue, below every threshold) — put "VAT registration + Fatoora-compliant invoicing" on the first-revenue checklist, expected around the 3-5th paying client.
...
- **🔴 Update 2026-07-22 — Email 2's question #5 (physical office requirement) is ANSWERED, new blocker surfaced.** NPHIES confirmed a physical KSA office/headquarters IS required for vendor certification, on top of the CR — see `docs/post-CR.md` §A1 for the full exchange and the human decision now needed (lease an office vs. investigate whether a virtual/registered-agent address qualifies). Questions 6-8 above (PKI, sandbox, conformance, clearinghouse) remain unanswered — the reply only addressed the office question.
...
|---|---|---|---|
| 1 | ~~Get annual-report PDFs~~ **DONE — no denial figure inside.** ~~File a formal Data Sharing Request~~ **SENT 2026-07-20 — Email 1**, auto-acknowledged (forwarded to relevant dept, no data/timeline yet); follow up by phone if quiet >1-2wk | **CHI DMO — `DMO@chi.gov.sa`** (CC `info@chi.gov.sa`; ☎ 19977 / 920001177) | A1, A2 |
...
| 12 | ~~Ask HealthOrbit/Ecaresoft actual pricing + current NPHIES depth~~ **HealthOrbit sent + replied 2026-07-20 (their KSA product isn't live until Q4 2026 — see D2); reply draft ready in Gmail.** Ecaresoft (**Email 11**) sent, no reply yet | **`sales@healthorbit.ai`** · **`sales@ecaresoft.com`** | D2 |
...
- **✅ Both forms submitted 2026-07-20/21.** Sales Support form: submitted, no reply yet. AWS Startups signup: **confirmed — welcome email received 2026-07-21** (to `i0562269690@gmail.com`, a different personal inbox than the outreach emails — note this for future AWS correspondence). Grants portal access to: **Activate Credits (up to $200,000, 200+ eligible services incl. AI/ML on Amazon Bedrock)**, an AI-powered "Startup Advisor" build assistant, migration planning, and startup-exclusive vendor offers. **This is portal access, not credits yet — applying for Activate Credits is a separate next step** (the "Apply for Activate Credits" link in that email), and it still doesn't answer the two original questions (real me-central-2 GPU/Trainium availability; whether Activate credits actually apply to GPU workloads in that specific region — Bedrock is a managed-model service, not raw GPU compute, so this may not even be the right credit category for a self-hosted-model plan).
- **What to do (remaining):** (1) click through and submit the actual Activate Credits application (not just the signup) — this is where GPU-specific eligibility would get clarified; (2) follow up on the unanswered Sales Support form if no reply in a few more days; (3) if the account manager or Activate application response doesn't address raw GPU-instance availability in `me-central-2` directly, ask explicitly — the credits offer doesn't imply GPU capacity exists there.
...
**✅ SENT, replied 2026-07-20 — see D2 above for the reply summary.** A response draft answering their 3 discovery questions is sitting in Gmail, ready to review and send.
...
| 4 | ~~Send AWS (Email 8) + CNTXT (Email 9)~~ **BOTH SENT 2026-07-19. AWS: Startups signup confirmed 2026-07-21 (portal access, $200k Activate Credits offer — see Email 8), Sales Support form still awaiting reply, Activate Credits application itself not yet submitted. CNTXT: no reply yet.** | Same logic as Oracle — evaluation/credits inquiry, not a contract |
...
**🟡 Update 2026-07-22 — #7 is IN PROGRESS, actively engaged (not yet a scheduled meeting).** Real KSA privacy counsel contact confirmed active over WhatsApp (a WhatsApp MCP connector is now available in this environment and is being monitored — see the hourly loop note below). Thread started 2026-07-21 17:19: counsel asked which parts of the project are on track vs. need adjusting, and specifically how hard it is to get approval for using US-hosted AI (part #2). After CR formation landed, counsel asked for a voice-note explanation of the project (something in a sent file wasn't clear); founder sent 4 voice notes explaining it. One was transcribed (Whisper, local — the WhatsApp MCP's media-download tool has a caching bug that returned identical bytes for all 4 message IDs, so only one was recoverable): it explains (1) the need for a data-processing agreement covering the data flow used to feed the product, and (2) the exact same open question already tracked as `C1`/`BLK-AI-1` — using a US-hosted AI provider (Anthropic) and whether that's hard to get approved, versus the in-Kingdom self-hosted fallback (`docs/05_open_source_switching.md`). Founder has since sent counsel the SDAIA-guideline-type documents and the NPHIES ToU PDF (both from `docs/counsel-docs/official-sources/`). See `docs/post-CR.md` §B1 for the same status in more detail, and `docs/counsel-docs/` for the full review pack to keep sending as counsel asks for more.

**Autonomous hourly loop (started 2026-07-22, user-requested):** Gmail and this WhatsApp thread are checked every hour; on a genuinely new counsel message, a next-step (requested document or an Arabic-drafted reply) is prepared and logged in `docs/counsel-whatsapp-log.md` for founder review — nothing is sent to counsel autonomously, founder has final say on every reply.
...
- [ ] **CST Cloud Computing Regulatory Framework applicability (G4, new 2026-07-20):** does a SaaS company that builds its own application and runs it on a third-party cloud (Oracle/AWS) — owning no data center or infrastructure itself — need to register with CST as a cloud computing service provider? CST's public registration criteria mention "directly or effectively controls the data center/critical infrastructure" as the trigger, but also say the framework was "expanded to expressly cover SaaS" with no clear carve-out found in public sources. Need a definitive read before GA.
