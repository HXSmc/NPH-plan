...
**Current status as of 2026-07-21: (A) STILL WAITING.** Emails 4 (Oracle) and 8 (AWS) were
both sent 2026-07-19; AWS's initial reply was boilerplate. Follow-up: Sales Support form
submitted (no reply yet), AWS Startups signup confirmed 2026-07-21 (portal access + a $200k
Activate Credits *offer* — not yet applied for, and Bedrock-framed rather than raw-GPU-framed,
so doesn't directly confirm `me-central-2` GPU/Trainium availability). **Neither the GPU
availability question nor a definitive credit answer exists yet** — see
`HUMAN_CONFIRMATION_NEEDED.md` Email 8 for exact status. Oracle: no reply yet. CST question is
logged in Section J, unanswered (no counsel engagement scheduled yet — also blocked on the counsel-meeting blocker
